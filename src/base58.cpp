...
namespace {
    class CNFTYAddressVisitor : public boost::static_visitor<bool> {
    private:
        CNFTYAddress *addr;
    public:
        CNFTYAddressVisitor(CNFTYAddress *addrIn) : addr(addrIn) { }
...
bool CNFTYAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CNFTYAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CNFTYAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CNFTYAddressVisitor(this), dest);
}

bool CNFTYAddress::IsValid() const {
...
CTxDestination CNFTYAddress::Get() const {
    if (!IsValid())
...
bool CNFTYAddress::GetKeyID(CKeyID &keyID) const {
...
bool CNFTYAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CNFTYSecret::SetKey(const CKey& vchSecret) {
...
CKey CNFTYSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CNFTYSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CNFTYSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CNFTYSecret::SetString(const std::string& strSecret) {
