...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NFTY
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NFTY
    // Mac: ~/Library/Application Support/NFTY
    // Unix: ~/.NFTY
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NFTY";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "NFTY.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "NFTYd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong NFTY will not work properly.");
