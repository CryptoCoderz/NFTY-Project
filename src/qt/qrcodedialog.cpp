...
{
    QString ret = QString("NFTY:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NFTY unit input in lnReqAmount, we generate the URI with NFTY as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(NFTYUnits::format(NFTYUnits::NFTY, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
