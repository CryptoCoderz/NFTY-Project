...
/*!
  ...
  To invert the direction of an axis, use \ref setRangeNFTYersed.
*/
...
/*!
  ...
  upper part.
*/
void QCPAxis::setRangeNFTYersed(bool reversed)
{
  if (mRangeNFTYersed != reversed)
  {
    mRangeNFTYersed = reversed;
    mCachedMarginValid = false;
  }
}
...
double QCPAxis::coordToPixel(double value) const
{
  if (orientation() == Qt::Horizontal)
  {
    ...
    } else // mScaleType == stLogarithmic
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeNFTYersed ? mAxisRect->right()+200 : mAxisRect->left()-200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeNFTYersed ? mAxisRect->left()-200 : mAxisRect->right()+200;
      else
      {
        if (!mRangeNFTYersed)
  ...
  } else // orientation() == Qt::Vertical
  {
    ...
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeNFTYersed ? mAxisRect->top()-200 : mAxisRect->bottom()+200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeNFTYersed ? mAxisRect->bottom()+200 : mAxisRect->top()-200;
      else
      {
        if (!mRangeNFTYersed)
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  mAxisPainter->reversedEndings = mRangeNFTYersed;
...
/*!
  ...
  \li range reversed (\ref QCPAxis::setRangeNFTYersed)
...
void QCPAxisRect::setupFullAxesBox(bool connectRanges)
{
  ...
  xAxis2->setRangeNFTYersed(xAxis->rangeNFTYersed());
  ...
  yAxis2->setRangeNFTYersed(yAxis->rangeNFTYersed());
...
void QCPColorScaleAxisRectPrivate::draw(QCPPainter *painter)
{
  ...
  {
    mirrorHorz = mParentColorScale->mColorAxis.data()->rangeNFTYersed() && (mParentColorScale->type() == QCPAxis::atBottom || mParentColorScale->type() == QCPAxis::atTop);
    mirrorVert = mParentColorScale->mColorAxis.data()->rangeNFTYersed() && (mParentColorScale->type() == QCPAxis::atLeft || mParentColorScale->type() == QCPAxis::atRight);
  }
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if (mAdaptiveSampling && dataCount >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
  {
    if (lineData)
    {
      ...
      int reversedFactor = keyAxis->rangeNFTYersed() ? -1 : 1; // is used to calculate keyEpsilon pixel into the correct direction
      int reversedRound = keyAxis->rangeNFTYersed() ? 1 : 0; // is used to switch between floor (normal) and ceil (reversed) rounding of currentIntervalStartKey
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  } else // valueAxis->mScaleType == QCPAxis::stLogarithmic
  {
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
      if ((valueAxis->range().upper < 0 && !valueAxis->rangeNFTYersed()) ||
          (valueAxis->range().upper > 0 && valueAxis->rangeNFTYersed())) // if range is negative, zero is on opposite side of key axis
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  {
    // x is key
    // if an axis range is reversed, the data point keys will be descending. NFTYerse them, since following algorithm assumes ascending keys:
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  double rectLeft = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().lower)-strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeNFTYersed()?-1:1));
  double rectRight = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().upper)+strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeNFTYersed()?-1:1));
  double rectBottom = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().lower)+strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeNFTYersed()?-1:1));
  double rectTop = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().upper)-strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeNFTYersed()?-1:1));
  int currentRegion;
...
void QCPBars::getVisibleDataBounds(QCPBarDataMap::const_iterator &lower, QCPBarDataMap::const_iterator &upperEnd) const
{
  ...
  while (it != mData->constBegin())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeNFTYersed() && barBounds.right() >= lowerPixelBound) || (mKeyAxis.data()->rangeNFTYersed() && barBounds.left() <= lowerPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeNFTYersed() && barBounds.top() <= lowerPixelBound) || (mKeyAxis.data()->rangeNFTYersed() && barBounds.bottom() >= lowerPixelBound));
    if (isVisible)
      lower = it;
    else
      break;
  ...
  while (it != mData->constEnd())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeNFTYersed() && barBounds.left() <= upperPixelBound) || (mKeyAxis.data()->rangeNFTYersed() && barBounds.right() >= upperPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeNFTYersed() && barBounds.bottom() >= upperPixelBound) || (mKeyAxis.data()->rangeNFTYersed() && barBounds.top() <= upperPixelBound));
    if (isVisible)
      upperEnd = it+1;
    else
      break;
...
void QCPBars::getPixelWidth(double key, double &lower, double &upper) const
{
  switch (mWidthType)
  {
    case wtAbsolute:
    {
      upper = mWidth*0.5;
      lower = -upper;
      if (mKeyAxis && (mKeyAxis.data()->rangeNFTYersed() ^ (mKeyAxis.data()->orientation() == Qt::Vertical)))
        qSwap(lower, upper);
      break;
    }
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode, const QSize &thumbSize)
{
  ...
  {
    bool mirrorX = (keyAxis()->orientation() == Qt::Horizontal ? keyAxis() : valueAxis())->rangeNFTYersed();
    bool mirrorY = (valueAxis()->orientation() == Qt::Vertical ? valueAxis() : keyAxis())->rangeNFTYersed();
