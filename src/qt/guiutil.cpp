...
{
    widget->setMaxLength(NFTYAddressValidator::MaxAddressLength);
    widget->setValidator(new NFTYAddressValidator(parent));
...
bool parseNFTYURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("NFTY"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NFTYUnits::parse(NFTYUnits::NFTY, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseNFTYURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to NFTY:
    //
    ...
    {
        uri.replace(0, 11, "NFTY:");
    }
    QUrl uriInstance(uri);
    return parseNFTYURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NFTY.lnk";
}
...
{
    return GetAutostartDir() / "NFTY.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a NFTY.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NFTY\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the NFTY app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add NFTY app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NFTY-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  NFTY-qt [" + tr("command-line options") + "]                     " + "\n";
