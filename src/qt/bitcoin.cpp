...
static void InitMessage(const std::string &message)
{
    if(splashref)
    {
        ...
        if(!fUseDarkTheme)
        {
            splashref->showMessage(QString::fromStdString(message), Qt::AlignBottom|Qt::AlignHCenter, QColor(0, 255, 198));
        }
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", NFTYGUI::tr("A fatal error occurred. NFTY can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    app.setOrganizationName("NFTY");
    //XXX app.setOrganizationDomain("");
    if(GetBoolArg("-testnet", false)) // Separate UI settings for testnet
        app.setApplicationName("NFTY-Qt-testnet");
    else
        app.setApplicationName("NFTY-Qt");
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit NFTY-Qt here
