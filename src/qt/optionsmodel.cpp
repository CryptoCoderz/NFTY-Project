...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", NFTYUnits::NFTY);
    ...
    if (!settings.contains("nAnonymizeNFTYAmount"))
        settings.setValue("nAnonymizeNFTYAmount", 1000);
    nAnonymizeNFTYAmount = settings.value("nAnonymizeNFTYAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNFTYAmount"))
        SoftSetArg("-anonymizeNFTYamount", settings.value("nAnonymizeNFTYAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNFTYAmount:
            return QVariant(nAnonymizeNFTYAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeNFTYAmount:
            nAnonymizeNFTYAmount = value.toInt();
            settings.setValue("nAnonymizeNFTYAmount", nAnonymizeNFTYAmount);
            emit AnonymizeNFTYAmountChanged(nAnonymizeNFTYAmount);
            break;
        default:
            break;
        }
